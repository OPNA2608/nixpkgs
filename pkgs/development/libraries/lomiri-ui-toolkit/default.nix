...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    sed -i -e 's|/usr|/run/current-system/sw|g' src/LomiriToolkit/i18n.cpp
    ...
    # statesaver test overrides XDG_RUNTIME_DIR with a custom path that it creates, in an attempt to ensure correct permissions
    # Ironically this causes a warning by Qt about the permissions being wrong (755 instead of 700), which the test suite treats as an error
    # Fix by setting the directory permissions
    sed -i \
      -e '/setenv("XDG_RUNTIME_DIR", testRuntimeDir.toUtf8()/a QFile::setPermissions(testRuntimeDir.toUtf8(), QFileDevice::ReadOwner | QFileDevice::WriteOwner | QFileDevice::ExeOwner);' \
    ...
    # lomiri-mobile rename comes from where? maybe distro-specific change, maybe they use a different Suru?
    substituteInPlace tests/unit/visual/tst_imageprovider.11.qml \
      --replace 'lomiri-mobile' 'ubuntu-mobile'
    ...
    for unexpectedWarningTest in ${lib.strings.concatStringsSep "" [
      ...
      # I don't know QML enough to fix this, upstream supposedly knows about this already
  ...
  postInstall = ''
    # Qt-generated pkg-config files have qtbase's prefix use correct QMAKE_PKGCONFIG_PREFIX, but wrong LIBDIR/INCDIR from qtbase
    # I think it's because these load the qt_module feature which overrides the variables in question with paths pointing into qtbase
