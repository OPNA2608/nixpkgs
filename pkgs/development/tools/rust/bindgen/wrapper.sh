...
targetflags="-target @defaultTarget@"
explicitAbiValue="@explicitAbiValue@"
if [[ -n "$explicitAbiValue" ]]; then
  targetflags="$targetflags -mabi=$explicitAbiValue"
fi
# shellcheck disable=SC2086
# cxxflags and NIX_CFLAGS_COMPILE should be word-split
exec -a "$0" @unwrapped@/bin/bindgen "$@" $sep $cxxflags @cincludes@ $targetflags $NIX_CFLAGS_COMPILE
